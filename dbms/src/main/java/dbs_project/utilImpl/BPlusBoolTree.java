...
public class BPlusBoolTree {
    ...
    class Node {
        ...
        public BPlusBoolTree.Node[] childNodes;
        public boolean isLeafNode;
        public BPlusBoolTree.Node nextNode;
    ...
    public int bulkLoad(BooleanArrayList entries) {
        int count = 0;
        for (int i = 0; i < entries.size(); i++) {
            if (!add(entries.get(i) ? 1 : 0, i + 1)) {
                count++;
            }
        }
            ...
            for (int i = 0; i < node.numKeys; i++) {
                if (node.keys[i] == key) {
                    ...
                    while (node.numKeys < N) {
                        ...
                        int curLength = node.keys.length;
                        int l = curLength;
                        for (; l <= N; l++) {
