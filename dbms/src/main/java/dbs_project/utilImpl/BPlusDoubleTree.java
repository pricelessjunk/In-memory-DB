...
public class BPlusDoubleTree {
    ...
    public class Node {
        ...
        public BPlusDoubleTree.Node[] childNodes;
        public boolean isLeafNode;
        public BPlusDoubleTree.Node nextNode;

        public Node() {
            numKeys = 0;
            keys = new double[2 * N - 1];
            values = new ArrayIntList[2 * N - 1];
            childNodes = new Node[2 * N];
        }
    }

    public BPlusDoubleTree() {
        globalRootNode = new Node();
        globalRootNode.isLeafNode = true;
    }

    public boolean add(double key, int object) {
    ...
    void insertIntoNonFullNode(Node node, double key, int object) {
    ...
    public ArrayIntList search(double key) {
        return search(globalRootNode, key);
    }

    public ArrayIntList search(Node node, double key) {
        while (node != null) {
            int i = 0;
    ...
    public int isElementPresent(double key) {
    ...
    public ArrayIntList getRangedValues(double fromKey, double toKey, boolean inclStartKey, boolean inclEndKey) {
        ArrayIntList ids = new ArrayIntList();
        double key = getNextGreaterKey(fromKey, inclStartKey);
    ...
    /* Node getLeafNodeForKey(double key) {
     Node node = globalRootNode;
     while (node != null) {
     int i = 0;
     while (i < node.numKeys && key > node.keys[i]) {
    ...
    /*public Node getLeafNodeForKey(double key) {
     Node node = globalRootNode;
     while (node != null) {
     int i = 0;
     if (!node.isLeafNode) {
    ...
    public int bulkLoad(ArrayDoubleList entries) {
        int count = 0;
    ...
    public double getNextGreaterKey(double key, boolean inclKey) {
        //Node node = globalRootNode;
        Node node = getLeafNodeForKey(key);
    ...
    public boolean remove(double key, int rowId) {
        Node node = getLeafNodeForKey(key);
